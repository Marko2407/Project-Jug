...
class MediaAsset(db.Model):
    ...
    storage_provider: Mapped[str] = mapped_column(
        Enum("DB", "GDRIVE", "EXTERNAL", name="storage_provider"), nullable=False
    )
    storage_path: Mapped[str] = mapped_column(db.Text, nullable=False)
    mime_type: Mapped[str | None] = mapped_column(db.String(255))
    bytes: Mapped[int | None] = mapped_column(db.Integer)
    checksum: Mapped[str | None] = mapped_column(db.String(255))
    width: Mapped[int | None] = mapped_column(db.Integer)
    height: Mapped[int | None] = mapped_column(db.Integer)
    ...
    thumbnail: Mapped["MediaAsset" | None] = relationship("MediaAsset", remote_side=[id])
